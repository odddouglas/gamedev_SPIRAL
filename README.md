...
## 1.玩家人物
- 八个方位移动，按住 `Shift` 不放移动可转换 **奔跑模式** 
- 人物可 **呼吸** ，移动时有少量动画，有生动性

## 2.镜子
- 有一面非实体镜子，但镜子的 **镜像** 只能面向前方，不能转向

## 3.背包
- 可储存物品，按 `B` 唤出或关闭，按 `Esc` 或箱子界面右上方图标也可关闭，打开背包时已设置 **不可移动** 
- 左键背包物品后，物品可随着鼠标的移动，视为 **拿取** 该物品
- 左键 **拿取** 物品后，可 **调换** 不同物品顺序，或 **叠加** 同类 **可叠加型物品** 数量。右键可使用 **消耗型物品** ，或 **拿取** 物品后在空地处放下一个该物品
- 有多个 **实体箱子** ，玩家人物靠近后按 `F` 唤出或关闭，按 `Esc` 或箱子界面右上方图标也可关闭，同时唤出背包，可与宝箱交换物品，左右键功能与背包一致，按 `Shift + 左键` 可 **快速移动** 物品至对方库存
- 不同箱子库存不同，玩家人物的背包可与不同的箱子交互
